...
lines += ["}", "", "impl Sprite {",
          ...
          "    #[must_use]", "    #[rustfmt::skip]", "    pub const fn rect(self) -> SrcRect {",
          "        match self {"]
for var, orig in variants:
    x, y, w, h = placements[orig]
    lines.append(f"            Self::{var} => SrcRect {{ x: {x}.0, y: {y}.0, w: {w}.0, h: {h}.0 }},")
lines += ["        }", "    }", "}", ""]
open(os.path.join(SIM, "sprite.rs"), "w", newline="\n", encoding="utf-8").write("\n".join(lines))
print("sim/src/sprite.rs", len(variants), "sprites,", "atlas", ATLAS_W, "x", atlas_h)
