//! Simulation constants, ported verbatim from the original Pascal source
//! (`reference/stuntcopter/StuntCopter.pas`). Keeping them in one place makes the
//! port auditable against the original and easy to calibrate.
//!
//! These are the *gameplay* constants — geometry, timing, and velocities the
//! simulation runs on. Pixel-exact HUD/ScoreBox layout lives with the UI, which
